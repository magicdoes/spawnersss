...
public final class MagicSpawnersCommand implements CommandExecutor, TabCompleter {
    ...
    private void give(CommandSender s,String[] a){
        ...
        if(amount<=0){s.sendMessage(plugin.messages().get("invalid-amount"));return;}var item=plugin.manager().createItem(type,amount);var rest=p.getInventory().addItem(item);rest.values().forEach(i->p.getWorld().dropItemNaturally(p.getLocation(),i));
