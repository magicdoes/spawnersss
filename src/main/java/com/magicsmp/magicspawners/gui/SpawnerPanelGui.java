...
public final class SpawnerPanelGui {
    ...
    public static void openDimension(Player p, World.Environment environment, int requestedPage) {
        ...
        // Bottom navigation row. Previous and Next are always visible so the layout is consistent.
        for (int i = 45; i < 54; i++) inv.setItem(i, SpawnerGui.button(Material.GRAY_STAINED_GLASS_PANE, " "));

        String prevLore = page > 0 ? "&aClick for previous page" : "&7No previous page";
        String nextLore = page < maxPage ? "&aClick for next page" : "&7No next page";
        inv.setItem(45, SpawnerGui.button(Material.ARROW, "&f← ᴘʀᴇᴠɪᴏᴜꜱ ᴘᴀɢᴇ", prevLore));
        inv.setItem(49, SpawnerGui.button(Material.BARRIER, "&cBack to Spawner Panel"));
        inv.setItem(53, SpawnerGui.button(Material.ARROW, "&fɴᴇxᴛ ᴘᴀɢᴇ →", nextLore));
